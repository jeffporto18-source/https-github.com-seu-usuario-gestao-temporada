...
import { Plus, Trash2, Sparkles, Pencil, Settings } from "lucide-react";
...
const emptyForm: InvestForm = { categoria: "", valor: "", descricao: "", competencia: "" };

export default function Investimentos() {
  const utils = trpc.useUtils();
  const { data: imoveis } = trpc.properties.list.useQuery();
  const { data: categorias } = trpc.investmentCategories.list.useQuery();
  ...
  const { data: itens, isLoading } = trpc.investments.list.useQuery(
    { propertyId: Number(propertyId), competencia },
    { enabled },
  );

  const total = useMemo(() => (itens ?? []).reduce((s, e) => s + Number(e.valor), 0), [itens]);

  const activeCategorias = useMemo(() => (categorias ?? []).filter(c => c.ativa === 1), [categorias]);

  const reset = () => { setForm(emptyForm); setEditId(null); };

  const create = trpc.investments.create.useMutation({
    onSuccess: () => { utils.investments.list.invalidate(); setOpen(false); reset(); toast.success("Investimento lançado."); },
    onError: (e) => toast.error(e.message),
  });

  const update = trpc.investments.update.useMutation({
    onSuccess: () => { utils.investments.list.invalidate(); setOpen(false); reset(); toast.success("Investimento atualizado."); },
    onError: (e) => toast.error(e.message),
  });

  const del = trpc.investments.delete.useMutation({
    onSuccess: () => { utils.investments.list.invalidate(); toast.success("Investimento removido."); },
    onError: (e) => toast.error(e.message),
  });

  const createCat = trpc.investmentCategories.create.useMutation({
    onSuccess: () => { utils.investmentCategories.list.invalidate(); setNewCatName(""); toast.success("Categoria criada."); },
    onError: (e) => toast.error(e.message),
  });

  const updateCat = trpc.investmentCategories.update.useMutation({
    onSuccess: () => { utils.investmentCategories.list.invalidate(); setEditingCatId(null); setEditingCatName(""); toast.success("Categoria atualizada."); },
    onError: (e) => toast.error(e.message),
  });

  const deleteCat = trpc.investmentCategories.delete.useMutation({
    onSuccess: () => { utils.investmentCategories.list.invalidate(); toast.success("Categoria removida."); },
    onError: (e) => toast.error(e.message),
  });

  const openEdit = (e: NonNullable<typeof itens>[number]) => {
    setEditId(e.id);
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <PageHeader
        title="Investimentos"
        subtitle="Enxoval e melhorias por unidade."
        action={
          <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) reset(); }}>
            <DialogTrigger asChild>
              <Button size="sm" disabled={!enabled} className="active:scale-[0.97] transition-transform">
                <Plus className="mr-1 h-3.5 w-3.5" /> Novo investimento
              </Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
                <DialogTitle className="font-serif">{editId ? "Editar investimento" : "Novo investimento"}</DialogTitle>
              </DialogHeader>
      ...
      {!enabled ? (
        <EmptyState title="Selecione um imóvel" subtitle="Escolha a unidade e a competência para lançar investimentos." />
      ) : isLoading ? (
        <div className="h-40 rounded-xl border border-border bg-card animate-pulse" />
      ) : !itens?.length ? (
        <EmptyState title="Nenhum investimento no período" />
      ) : (
        <Card className="overflow-hidden">
            ...
            {itens.map((e) => (
              <div key={e.id} className="flex items-center justify-between px-5 py-3.5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-sm">{e.categoria}</p>
          ...
          <div className="flex items-center justify-between px-5 py-4 bg-secondary/50 border-t border-border">
            <span className="text-sm font-medium">Total investido no período</span>
